                ...
                <article class={`card-content article${'direction' in page ? ' ' + page.direction : ''}`} role="article">
                    ...
                    {page.layout !== 'page' ? <div class="article-meta is-size-7 is-uppercase level is-mobile">
                        <div class="level-left">
                            {page.top > 0 ?
                                <div class="level-item tag is-danger" style="background-color: #ff1700;;"><i class="fa fa-thumbtack"></i></div> : null}
                            {/* Creation Date */}
                            {page.date && <span class="level-item" dangerouslySetInnerHTML={{
                                __html: `<i class="far fa-calendar-alt">&nbsp;</i>` + _p('article.created_at', `<time dateTime="${date_xml(page.date)}" title="${new Date(page.date).toLocaleString()}">${date(page.date)}</time>`)
                            }}></span>}
                            {/* Last Update Date */}
                            {page.updated && <span class="level-item" dangerouslySetInnerHTML={{
                                __html: `<i class="far fa-calendar-check">&nbsp;</i>` + _p('article.updated_at', `<time dateTime="${date_xml(page.updated)}" title="${new Date(page.updated).toLocaleString()}">${date(page.updated)}</time>`)
                            }}></span>}
                            {/* author */}
